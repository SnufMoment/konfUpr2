...
--repo	URL репозитория (https://api.nuget.org/v3/index.json) или путь к test-repo.json (обязательный).
--mode	Режим работы: online или test (обязательный).
--max-depth	Максимальная глубина анализа зависимостей (обязательный, ≥0).
--filter	Подстрока для фильтрации пакетов (опционально, исключает пакеты, содержащие подстроку).
4. Сборка и запуск
...
отображается граф зависимостей и порядок установки,

генерируется SVG-файл с визуализацией.

6. Примеры работы
6.1. Граф зависимостей (пример: AutoMapper)
Dependency graph:
...
SVG-файл автоматически создаётся и отображает направленный граф зависимостей.
...
автоматическая визуализация в SVG-формате.

Все функции протестированы на реальных и тестовых пакетах и соответствуют требованиям задания, не используя запрещённые инструменты.
```
